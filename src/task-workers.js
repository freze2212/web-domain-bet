...
import { assignDomain } from "./ownership.js";
import { deductBalance, topupBalance } from "./wallet.js";
import { addHistoryItem, updateHistoryItem } from "./history.js";
...
export async function executeSwitchMode(taskId) {
  ...
  try {
    ...
    if (toMode === "302") {
      ...
      updateTaskProgress(taskId, 65, "Đang cấu hình Page Rule 302 & DNS 8.8.8.8...", `Đích đến: ${targetUrl}`, "info");
      const cfRes = await setupDirect302Redirect(domain, targetUrl);
      ...
      addHistoryItem({
        ...
        status: "success",
        isBuy: false,
        cfAccount: cfRes?.accountName || "Cloudflare",
        error: null,
        taskId,
        userId,
        username,
      ...
      const repoMatches = findDomainInRepos(domain);
      for (const m of repoMatches) {
        // Giữ lại nếu đã đúng template đích — vẫn remove rồi add lại để sạch
        await removeDomainFromRepo(domain, m.filePath || m.folderPath).catch(() => {});
      }
      await removeDomainFromAllPagesProjects(domain).catch(() => {});
      ...
      const finalTarget = cfRes?.target || template.cnameTarget;
      ...
      addHistoryItem({
        ...
        actionLabel: "Chuyển sang Landing Page",
      ...
      setTimeout(() => verifyHistoryItem(historyId).catch(() => {}), 2000);
      completeTask(taskId, { domain, toMode: "LP", templateName: template.name, targetUrl: finalLink, cnameTarget: finalTarget }, `Đã chuyển ${domain} sang Landing Page [${template.name}] thành công!`);
    }
  } catch (err) {
    console.error(`[Worker] Lỗi SwitchMode cho ${domain}:`, err);
    failTask(taskId, err, `Lỗi chuyển đổi chế độ cho ${domain}`);
  }
}
...
export async function executeUpdateLink(taskId) {
  ...
  try {
    ...
    completeTask(
      taskId,
      { domain, newLink, details: res },
      `Đã cập nhật link đích thành công cho ${domain}!`
    );
  } catch (err) {
