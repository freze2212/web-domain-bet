/**
 * Point domains to origin IP: remove Pages custom domains, set A records, purge.
 */
...
async function pointDns(zone, tokenMode, domain) {
  ...
  for (const name of [domain, 'www.' + domain]) {
    const body = { type: 'A', name, content: WANT_IP, proxied: true, ttl: 1 };
    try {
      ...
      console.log('  ADD A', name, '->', WANT_IP, 'id=' + (created.result?.id || ''));
    } catch (e) {
      console.log('  ADD_ERR', name, e.message);
    }
  }
  try {
    const purgeBody = { purge_everything: true };
    if (tokenMode === 'admin') await cfAdmin('/zones/' + zid + '/purge_cache', { method: 'POST', body: purgeBody });
    else await cfRequestFull('/zones/' + zid + '/purge_cache', { method: 'POST', body: purgeBody });
    console.log('  PURGE ok');
  } catch (e) {
    console.log('  PURGE_ERR', e.message);
  }
}
...
for (const domain of DOMAINS) {
  ...
  try {
    ...
    // remove Pages on Freze always (domains often attached there even if zone elsewhere)
...
const c = new Client();
c.on("ready", () => {
  c.exec(`echo '${b64}' | base64 -d > /tmp/_batch_point_ip.mjs && node /tmp/_batch_point_ip.mjs`, (e, s) => {
    let o = "";
    s.on("data", (d) => (o += d));
